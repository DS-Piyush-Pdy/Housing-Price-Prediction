...
if not os.path.exists(MODEL_FILE):
    ...
    for train_index, test_index in split.split(data, data["income_cat"]):
        data.loc[test_index].drop("income_cat", axis = 1).to_csv("input.csv", index = False) 
        data = data.loc[train_index].drop("income_cat", axis = 1)  # Dropping the income_cat column as it is not needed anymore.
